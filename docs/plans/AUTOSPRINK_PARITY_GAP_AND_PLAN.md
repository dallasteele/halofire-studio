...
- ~~No **real hydraulic network solve**~~ — **DONE (Phase 3, 2026-06-14):** real connected-network Hazen-Williams solve over the Phase-2 topology, remote-area demand subset, iterative pipe upsizing, fitting Le, named/multiple supplies, P/Q/V node tags, color-by-condition, live re-calc, NFPA-format report — all golden-tested AND independently hand-verified to EXACT agreement, camera-neutral, deployed. **The one honest PARTIAL is the Hardy-Cross loop balance: the solver is proven correct (textbook 2-loop → Newton-derived flows, Kirchhoff holds) but is not yet wired to the live demand tree** (flagged in-product, not faked).
...
| AutoSPRINK tool | our status | what's missing | eff | pri |
|---|---|---|---|---|
| **Hazen-Williams network solve** | HAVE | real connected-network solver (Phase 3, 2026-06-14): buildGraph splits pipes at interior taps into one connected component, BFS spanning tree from supply, junction flow conservation, Q=K√P heads, HW friction + 0.433 psi/ft elev + Pv, iterative pipe upsizing. 30 golden + 139/139 engine suite + live verified (conservationOk, 18223.9 gpm / 466.5 psi on 1230 heads). **Independent hand-calc agrees EXACT** (HW loss 1.98059 psi/ft seg0, requiredPsi reconstruction 435.029, upsizing 156.146→74.238). Residual: default basis is ALL-HEADS-OPEN worst-case envelope, NOT remote-area demand (the remote-area tool feeds the subset) — labeled in-product | — | — |
| **Hardy-Cross loop/grid balance** | PART | solver is REAL + proven (Phase 3): findLoops cyclomatic μ=E−N+comp, ΔQ=−Σh_f/(n·Σ\|h_f\|/\|Q\|) per loop to Σh_f→0; 28 golden + LIVE in-browser textbook 2-loop converges to **independently Newton-derived flows** (59.557/40.443/19.114 gpm, Kirchhoff 1st+2nd laws hold, maxResidual<1e-9). **HONEST PARTIAL: the live 1230-head model's one gridded loop is NOT yet wired to the demand tree** (analyzeGraphLoops reports converged:false / maxResidual:null — flagged in-product, not faked). Wiring head-discharge demands into the loop balance is the Phase-3 followup | M | P1 |
...
| **NFPA-format hydraulic report** | HAVE | buildHydraulicReport mirrors a solveNetwork() result into NFPA-format summary + node-analysis (Ref/Elev/K/P/Q) + pipe-data (From→To/dia/len+Le/effLen/C/Q/V/loss) rows, K only on sprinkler nodes, printable self-contained HTML export (Phase 3): 26 golden + 188/188 hydraulics suite + 28/28 live. **PURE mirror of the validated solve — node-row pressures Δ=0 and pipe-row friction/velocity Δ=0 over 2543 rows vs the live per-element solve; hand-calc agrees EXACT** (P1 friction 6.5452 psi, P2 2.2766 psi, v 20.175 fps). Residual: default report basis is all-heads-open envelope (flips to remote-area-subset once a boundary is drawn); inherits the solve's PARTIAL Hardy-Cross flag on looped mains; in-panel preview samples 4+4 rows (full set in the .html export) | — | — |
...
5. **Real hydraulic solve** — ✅ DONE (Phase 3): system-wide Hazen-Williams + fitting Le, golden + hand-verified EXACT. **Hardy-Cross** loop/grid solver is built + proven on the textbook net but its live-model loop-to-demand wiring is PARTIAL (honestly flagged).
...
**Phase 3 — Hydraulics that hold up** *(the engineering)* — ✅ **DONE** except Hardy-Cross live wiring (live-verified, 2026-06-14)
...
**Is the hydraulic solve trustworthy?** YES, as an **ENGINEERING AID — NOT AHJ/PE-stamped, NOT a permit-ready stamped calc** (labeled throughout the product). The forward demand-fixed Hazen-Williams tree solve, fitting Le, remote-area demand, supplies, node tags, color-by-condition and NFPA report are all golden-tested AND independently hand-verified to exact agreement, with flow conservation holding at every junction and camera-neutrality proven. The honest limits the engineer must read: (1) the default basis flows **ALL heads open** (a worst-case envelope) unless a Remote-Area boundary is drawn to restrict it to the NFPA-13 design subset; (2) looped/gridded mains are walked as a BFS tree — the **Hardy-Cross GRID balance is built and proven on the textbook net but not yet wired to the live demand tree** (`hardyCrossBalanced=false`, `loopCount` surfaced), so on looped systems the per-node pressures are the honest PARTIAL tree result, not a fully balanced grid; (3) the Le tables and density curves are representative public NFPA-13 values, not a manufacturer-certified database. No claim gate was cleared; every output carries the engineering-aid disclaimer in-product. NOTE: a spec-cited fixture ("100 gpm, C=120, 2″ sch40 → ~0.0479 psi/ft") was found WRONG — the mathematically correct Hazen-Williams value is **0.09390 psi/ft** (0.0479 ≈ a 2.4″ ID); the engine + golden tests correctly use 0.0939 and document the spec error. This is a spec-fixture error, not an engine bug.

**Next: Phase 4 — Intake & deliverables** (PDF/DWG → building model + scale calibration · section/elevation cut → 2D views · submittal sheet set FP-0…FP-D + titleblock plot · drop-ceiling tile grid · cut-sheet bundle). The natural first Phase-4-adjacent followup that closes the remaining Phase-3 PARTIAL is **wiring the Hardy-Cross loop balance into the live demand tree** (assign head-discharge demands around the gridded loop and iterate Σh_f→0), which would flip `hardyCrossBalanced` true on looped systems.
