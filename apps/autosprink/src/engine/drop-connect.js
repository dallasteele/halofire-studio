  ...
  for (const h of heads) {
    ...
    if (c.armOver) {
      ...
      if (len3(foot, turn) > NODE_TOL_FT) {
        ...
        solids.push(fitting('fitting_elbow_90', foot, added.length));
        added.push(solids[solids.length - 1]);
        solids.push(fitting('fitting_elbow_90', turn, added.length));
        added.push(solids[solids.length - 1]);
      }
    }
    ...
    if (len3(turn, p) > NODE_TOL_FT * 0.5) {
      ...
      solids.push(fitting('fitting_reducer', [round(p[0]), round(p[1]), round(p[2])], added.length));
      added.push(solids[solids.length - 1]);
    }
  }
...
function fitting(componentKey, position, n) {
  return {
