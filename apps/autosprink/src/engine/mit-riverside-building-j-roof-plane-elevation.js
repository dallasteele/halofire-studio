...
export function renderMitRiversideBuildingJRoofPlaneElevation(packet) {
  ...
  return { status: 'passed', topSvg, elevationSvg, model3dSvg, counts: { main: main.length, membrane: membrane.length, pending: pending.length }, surfaces: region('main-standing-seam') && packet.roofSurfaces.length, headInstallationZReady: false, complianceReady: false };
}
