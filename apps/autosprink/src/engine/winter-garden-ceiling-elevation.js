...
export async function validateWinterGardenCeilingElevationEvidence(value) {
  ...
  if (fabrication.pipingRows !== 316 || fabrication.outletRows !== 338 || fabrication.pendentHalfInchOutletRows !== 158 || fabrication.branch25PieceRows !== 15 || fabrication.spatialHeadMappingReady !== true || fabrication.mappedPendentTakeoffRows !== 15 || fabrication.branchRowPipeElevationReady !== true || fabrication.manufacturerInstallationEnvelopeReady !== true || fabrication.spatialMappingReceiptSha256 !== 'c746308eddaf57f1c17da5b128744c3ccbfd596240ba825335bd0f3a677d7673') issues.push(issue('WG_CEILING_FABRICATION_RECEIPT_DRIFT', 'SprinkCad inventory, 15-head spatial mapping, branch Z, or manufacturer envelope receipt changed.'));
...
export function renderWinterGardenCeilingViews(model) {
  ...
  const fabricationRegistered = model.fabricationPlanMappingReady === true && model.manufacturerInstallationEnvelopeReady === true;
  ...
  const branchPipes = (model.branchPipes3d || []).map((pipe) => { const x = 80 + (pipe.fromFt[1] - minY) * 24; const y = 640 - (pipe.elevationFt - 114) * 28; return `<circle cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="7" fill="#a78bfa"><title>${pipe.sourceText} branch centerline EL ${pipe.elevationFt.toFixed(3)} FT</title></circle>`; }).join('');
  const topStatus = fabricationRegistered ? '15 completed FP3 heads / 15 SprinkCad 1-inch takeoffs / TY3531' : '15 completed FP3 heads / absolute C8 ceiling surface / deflector offset unresolved';
  const elevationStatus = fabricationRegistered ? 'C8 + FP2 branch Z + submitted TFP181 3/16-11/16 in dropped-deflector envelope' : 'C8 15\'-0 9/16&quot; to 20\'-6 3/4&quot; / 4.5:12 / orange = unresolved deflector interval';
  return { status: 'passed', topSvg: `<svg viewBox="0 0 1200 1000" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Winter Garden source-bound pitched ceiling top view"><rect width="100%" height="100%" fill="#07111f"/>${topHeads}<text x="24" y="36" fill="#e0f2fe" font-family="monospace" font-size="18">${topStatus}</text></svg>`, elevationSvg: `<svg viewBox="0 0 1200 720" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Winter Garden source-bound pitched ceiling elevation envelope"><rect width="100%" height="100%" fill="#07111f"/><polyline points="80,610 410,455 740,610" fill="none" stroke="#38bdf8" stroke-width="5"/>${branchPipes}${elevationHeads}<text x="24" y="36" fill="#e0f2fe" font-family="monospace" font-size="18">${elevationStatus}</text></svg>`, complianceReady: false };
}
