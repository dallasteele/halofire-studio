...
test('detectColumnMarkers keeps one marker per filled blob at the true blob position', () => {
  ...
  const { markers } = detectColumnMarkers(segments, { markerRadiusFt: 1.4, minMarkerSegs: 6, markerMaxLenFt: 1.5 });

  assert.equal(markers.length, expected.length);
  assert.deepEqual(markers.map((marker) => ({ x: marker.x, y: marker.y })), [...expected].sort(byPosition));
});
...
test('detectColumns emits one standing column per real marker and never fills the grid', () => {
  ...
  const segments = expected.flatMap(({ x, y }) => filledMarkerBlob(x, y));
  const { columns } = detectColumns(grid, segments, [], { markerRadiusFt: 1.4, minMarkerSegs: 6, markerMaxLenFt: 1.5, snapRadiusFt: 1.2 });
