const EXPECTED = Object.freeze({
  ...
  pieceVectorHoldoutIds: [],
});
const EXPECTED_FINGERPRINT = '866fa90276742cc6';
const EXPECTED_METRICS = Object.freeze({
  ...
  fieldAsBuiltAlternateTwinCenterlineCount: 71,
...
function pieceVectorCloses(row, evidence) {
  ...
  const heavy = row.sourceCenterline?.widthPt === 1.24059
  ...
  const alternate = row.sourceCenterline?.widthPt === 0.01389
    && row.sourceCenterline.representation === 'black-white-twin-centerline'
    && Number.isInteger(row.sourceCenterline.fieldBlackDrawingIndex)
    && Number.isInteger(row.sourceCenterline.fieldWhiteDrawingIndex)
    && Number.isInteger(row.sourceCenterline.asBuiltBlackDrawingIndex)
    && Number.isInteger(row.sourceCenterline.asBuiltWhiteDrawingIndex)
    && row.sourceCenterline.blackItemIndex === 0
    && row.sourceCenterline.whiteItemIndex === 0
    && row.mappingBasis === 'exact-source-spatial-label-native-cut-length-black-white-twin-centerline-cross-source'
    && row.labelAssociationBasis === 'exact-source-spatial-line-grouping'
...
export function evaluateNewHopeWetWeldedBranchRegistrationEvidence(evidence = {}) {
  ...
  const heavyCount = evidence.pieceVectorMappings?.filter((row) => row.sourceCenterline?.widthPt === 1.24059).length ?? 0;
  const alternateCount = evidence.pieceVectorMappings?.filter((row) => row.sourceCenterline?.widthPt === 0.01389).length ?? 0;
  if (labelIds.length !== 71 || new Set(labelIds).size !== 71 || pieceVectorIds.length !== 71 || new Set(pieceVectorIds).size !== 71 || !same(pieceVectorHoldoutIds, EXPECTED.pieceVectorHoldoutIds) || new Set([...pieceVectorIds, ...pieceVectorHoldoutIds]).size !== 71 || sourceSegmentKeys.length !== 71 || new Set(sourceSegmentKeys).size !== 71 || heavyCount !== 67 || alternateCount !== 4) issues.push(issue('NH_WET_BRANCH_PIECE_VECTOR_COVERAGE_INVALID', 'pieceVectorMappings', 'Exactly 71 welded units must consume 71 unique field/as-built source segments once: 67 heavy centerlines plus four BL03/BL04 black-white-twin centerlines.'));
  ...
  })) issues.push(issue('NH_WET_BRANCH_DIRECTION_VECTOR_BINDING_INVALID', 'registrations', 'Every direction registration must bind the same source centerline in the 71-unit welded piece-vector mapping.'));
  const claims = evidence.claims;
  if (claims?.weldedBranchLabelInventoryReady !== true || claims?.fieldAsBuiltHeavyCenterlineParityReady !== true || claims?.fieldAsBuiltAlternateTwinCenterlineParityReady !== true || claims?.weldedBranchPieceVectorBijectionReady !== true || claims?.scopedPieceToPlanVectorMappingReady !== true || claims?.scopedFabricationStationDirectionReady !== true || claims?.completeWeldedBranchPieceMappingReady !== true || claims?.pieceToPlanVectorMappingReady !== false || claims?.hydraulicFlowDirectionReady !== false || claims?.gradeReady !== false || claims?.installedElevationReady !== false || claims?.fabricationReady !== false || claims?.fieldReleaseReady !== false) issues.push(issue('NH_WET_BRANCH_REGISTRATION_FALSE_PROMOTION', 'claims', 'The complete 71-unit welded plan mapping and 15-unit station direction cannot promote global piece mapping, hydraulic direction, grade, elevation, fabrication, or field release.'));
  ...
  return {
    ...
    fieldAsBuiltAlternateTwinCenterlineParityReady: ready,
    ...
    completeWeldedBranchPieceMappingReady: ready,
