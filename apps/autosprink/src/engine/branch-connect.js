...
import { createFittingPlacement, normalizeRunAxis } from './fitting-orient.js';
...
export function repairConnectivity(cadModel) {
  ...
  for (;;) {
    ...
    const tee = {
      ...
      autoGenerated: true,
      placement: createFittingPlacement(
        'fitting_tee',
        normalizeRunAxis([
          best.toPt[0] - best.fromPt[0],
          best.toPt[1] - best.fromPt[1],
          0,
        ]),
        [0, 0, best.toPt[2] >= best.fromPt[2] ? 1 : -1],
        { fittingType: 'auto-tie-tee' },
      ),
    };
    solids.push(tee);
    added.push(tee);
