...
import { evaluateNewHopeNativeHangerSchedule } from '../../../engine/new-hope-native-hanger-schedule.js';
...
const nativeFabHangerScheduleUrl = '../../new-hope-native-fab-hanger-schedule.json';
...
try {
  ...
  if (!nativeFabHangerScheduleResponse.ok) throw new Error(`native FAB hanger schedule fetch ${nativeFabHangerScheduleResponse.status}`);
  ...
  const [calibration, source, pipeVectors, planGraph, operationalAnnotations, hydraulicRoute21Evidence, hydraulicRoute22Evidence, hydraulicRoute23Evidence, architecturalSource, elevationDatumSource, atticSource, atticCalibration, answerEvidence, sourceFeedAsbuiltRiserRegistration, fabricationEndScheduleSource, nativeFabTopology, nativeFabAttachmentGraphSource, completedProjectFittingAdjacencyCalibration, purchasedSupportComponentsSource, nativeFabHangerScheduleSource, threadedTerminalCatalogPartsSource] = await Promise.all([response.json(), sourceResponse.json(), pipeVectorResponse.json(), planGraphResponse.json(), operationalResponse.json(), hydraulicRoute21Response.json(), hydraulicRoute22Response.json(), hydraulicRoute23Response.json(), architecturalSourceResponse.json(), elevationDatumResponse.json(), atticSourceResponse.json(), atticCalibrationResponse.json(), answerEvidenceResponse.json(), sourceFeedAsbuiltRiserResponse.json(), fabricationEndScheduleResponse.json(), nativeFabTopologyResponse.json(), nativeFabAttachmentGraphResponse.json(), completedProjectFittingAdjacencyCalibrationResponse.json(), purchasedSupportComponentsResponse.json(), nativeFabHangerScheduleResponse.json(), threadedTerminalCatalogPartsResponse.json()]);
  const scale = 1.5;
  const branchY = 430;
  ...
  const nativeHangerSchedule = evaluateNewHopeNativeHangerSchedule({ schedule: nativeFabHangerScheduleSource, purchasedSupportComponents: purchasedSupportComponentsSource });
  if (!nativeHangerSchedule.nativeHangerScheduleReady) throw new Error(`native hanger schedule: ${nativeHangerSchedule.blockerCodes.join(', ')}`);
  ...
  document.querySelector('#threaded-terminal-catalog-part-status').textContent = `CATALOG/STANDARD IDENTITY PASS / EXACT ASSEMBLY REJECTED: New Hope quote 0133820 binds ASC/SCI Fig 3201 product ${threadedTerminalCatalogParts.parts[0].purchaseProductNumber} at A=${threadedTerminalCatalogParts.parts[0].publishedDimensionsIn.centerToEndA.toFixed(2)} in; its hash-bound ${threadedTerminalCatalogParts.parts[0].officialSubmittalRevision} says ${threadedTerminalCatalogParts.parts[0].bodyDimensionStandard}. Fig 3221R product ${threadedTerminalCatalogParts.parts[1].purchaseProductNumber} is A=${threadedTerminalCatalogParts.parts[1].publishedDimensionsIn.overallLengthA.toFixed(2)} in, but its hash-bound ${threadedTerminalCatalogParts.parts[1].officialSubmittalRevision} says ${threadedTerminalCatalogParts.parts[1].bodyDimensionStandard} while ASC's current product page says ${threadedTerminalCatalogParts.parts[1].productPageBodyDimensionStandard}; the body-standard conflict remains blocked. Both require ${threadedTerminalCatalogParts.parts[0].threadStandard} ${threadedTerminalCatalogParts.parts[0].threadForm}. The reusable gate now requires ${threadedTerminalCatalogParts.metrics.exactAssemblyRequiredPartDefinitionCount} part-number-specific watertight solids and all ${threadedTerminalCatalogParts.metrics.exactAssemblyRequiredInstalledUnitCount} CMI.23-CMI.42 fitting instances, with full critical-dimension audits, modeled helical threads, engagement/clearance checks, exact-BREP fit receipts, installed-scene collision receipts, and zero untrusted promotions. Current installed exact instances: ${threadedTerminalCatalogParts.metrics.exactAssemblyInstalledInstanceCount}; blockers: ${threadedTerminalCatalogParts.exactAssemblyBlockerCodes.join(', ')}`;
  document.querySelector('#purchased-support-component-status').textContent = `SCHEDULE CROSSWALK PASS / INSTALLED MODEL REJECTED: native Project.seidb maps ${nativeHangerSchedule.metrics.nativeHangerQuantity} hangers across ${nativeHangerSchedule.metrics.scheduleRowCount} exact line/size rows and ${nativeHangerSchedule.metrics.lineCount} fabrication lines to five quote-bound ASC Fig. 69 product variants, including each published nominal-size dimension table. Quote 0133821 binds ${purchasedSupportComponents.metrics.purchasedSupportLineCount} support product lines / ${purchasedSupportComponents.metrics.purchasedSupportUnitCount} units, including quantity parity between 212 Fig. 69 hangers and 212 Sammy anchors. Two official Victaulic AB2 Revit family containers are hash-bound and their 146 PartAtom variations are audited, but the quote-bound A240AB200N product number occurs zero times and no thread-geometry parameter is present. The supposed ASC/Anvil Fig. 69 RFA is machine-classified as an HTML catalog response saved with an RFA filename and is rejected; ${purchasedSupportComponents.metrics.fig69PublishedVariantCount} catalog size variants remain dimension evidence only. Four official ASC AF730/AF035/AF076/AF779 submittals bind eight quote products and the actual mating rules: brace pipe bottom-out or one-inch extension, Schedule 40 brace size, H1/H2 attachment, structure fasteners, torque-off hardware, and ASC-only listed assembly compatibility. The hash-bound bid index audits ${purchasedSupportComponents.metrics.indexedCadCorpusFileCount} files, including ${purchasedSupportComponents.metrics.indexedCadCorpusCadFileCount} CAD-capable files, with ${purchasedSupportComponents.metrics.indexedCadCorpusExactTargetMatchCount} exact target filename/path matches; 18 generic hanger or brace SCAD rows remain ineligible. ASC advertises STEP/IGES/SAT/DXF/Revit downloads, but its live MultiCAD lookup returned none of the eight quote product numbers and requires an email request; no request or valid download is claimed. The reusable exact-part gate requires ${purchasedSupportComponents.metrics.exactAssemblyRequiredPartDefinitionCount} source-bound part solids across ${purchasedSupportComponents.metrics.exactAssemblyRequiredInstalledUnitCount} installed units, independently trusted geometry hashes and critical-dimension audit hashes, modeled thread profile/lead/crest/root solids, verified engaged turns, dimensional clearance, structure capacity and angle, exact-BREP fit receipts, and installed-scene collision receipts. Current source-trusted part definitions: ${purchasedSupportComponents.metrics.exactAssemblySourceTrustedPartDefinitionCount}; exact installed instances: ${purchasedSupportComponents.metrics.exactAssemblyInstalledInstanceCount}. Catalog images, generated proxies, caller flags, self-asserted hashes, and untrusted receipts cannot release it. ASC price-sheet page 108 corrects product 0500604541 to AF779 catalog size 2. Official ITW SWDR IGES was machine-audited across ${purchasedSupportComponents.metrics.sammyIgesEntityCount} entities: 274 curves, two planar surfaces, zero solid primitives, zero BREP/topology entities, and every sampled geometry Z at zero. It is reproducibly planar application line art, not a part-number-specific thread-bearing solid. The quote says side steel while as-built FP0.1 shows wood-side and wood-vertical Sammy details; exact solids, thread geometry, fitted connections, structure attachment, collision analysis, and installed placement remain unresolved`;
  ...
  document.querySelector('#machine-acceptance-boundary').textContent += ` | threadedTerminalCatalogPartIdentityReady=${threadedTerminalCatalogParts.catalogPartIdentityReady} | threadedTerminalBodyDimensionStandardsIdentified=${threadedTerminalCatalogParts.bodyDimensionStandardsIdentified} | threadedTerminalBodyDimensionStandardConflictResolved=${threadedTerminalCatalogParts.bodyDimensionStandardConflictResolved} | threadedTerminalThreadStandardIdentified=${threadedTerminalCatalogParts.threadStandardIdentified} | threadedTerminalManufacturerPrimaryDimensionsReady=${threadedTerminalCatalogParts.manufacturerPrimaryDimensionsReady} | threadedTerminalManufacturerSecondaryEnvelopeReady=${threadedTerminalCatalogParts.manufacturerSecondaryEnvelopeReady} | threadedTerminalExactInternalThreadFormReady=${threadedTerminalCatalogParts.exactInternalThreadFormReady} | threadedTerminalExactThreadEngagementReady=${threadedTerminalCatalogParts.exactThreadEngagementReady} | threadedTerminalExactAssemblyPartDefinitionsReady=${threadedTerminalCatalogParts.exactAssemblyPartDefinitionsReady} | threadedTerminalExactAssemblyInstalledInstanceCoverageReady=${threadedTerminalCatalogParts.exactAssemblyInstalledInstanceCoverageReady} | threadedTerminalExactAssemblyConnectionFitReady=${threadedTerminalCatalogParts.exactAssemblyConnectionFitReady} | threadedTerminalExactAssemblySolidKernelReceiptReady=${threadedTerminalCatalogParts.exactAssemblySolidKernelReceiptReady} | threadedTerminalExactAssemblySceneCollisionReceiptReady=${threadedTerminalCatalogParts.exactAssemblySceneCollisionReceiptReady} | threadedTerminalExactAssemblyReleaseReady=${threadedTerminalCatalogParts.exactAssemblyReleaseReady}`;
  ...
  document.documentElement.dataset.threadedTerminalBodyDimensionStandardsIdentified = String(threadedTerminalCatalogParts.bodyDimensionStandardsIdentified);
  document.documentElement.dataset.threadedTerminalBodyDimensionStandardConflictResolved = String(threadedTerminalCatalogParts.bodyDimensionStandardConflictResolved);
  document.documentElement.dataset.threadedTerminalThreadStandardIdentified = String(threadedTerminalCatalogParts.threadStandardIdentified);
  ...
  document.documentElement.dataset.threadedTerminalExactAssemblyPartDefinitionsReady = String(threadedTerminalCatalogParts.exactAssemblyPartDefinitionsReady);
  document.documentElement.dataset.threadedTerminalExactAssemblyInstalledInstanceCoverageReady = String(threadedTerminalCatalogParts.exactAssemblyInstalledInstanceCoverageReady);
  document.documentElement.dataset.threadedTerminalExactAssemblyConnectionFitReady = String(threadedTerminalCatalogParts.exactAssemblyConnectionFitReady);
  document.documentElement.dataset.threadedTerminalExactAssemblySolidKernelReceiptReady = String(threadedTerminalCatalogParts.exactAssemblySolidKernelReceiptReady);
  document.documentElement.dataset.threadedTerminalExactAssemblySceneCollisionReceiptReady = String(threadedTerminalCatalogParts.exactAssemblySceneCollisionReceiptReady);
  document.documentElement.dataset.threadedTerminalExactAssemblyReleaseReady = String(threadedTerminalCatalogParts.exactAssemblyReleaseReady);
  document.documentElement.dataset.purchasedSupportIdentityReady = String(purchasedSupportComponents.purchaseIdentityReady);
  document.documentElement.dataset.nativeHangerScheduleReady = String(nativeHangerSchedule.nativeHangerScheduleReady);
  document.documentElement.dataset.nativeHangerPerLineSizeAssignmentReady = String(nativeHangerSchedule.perLineNominalPipeSizeAssignmentReady);
  document.documentElement.dataset.nativeHangerCatalogDimensionAssignmentReady = String(nativeHangerSchedule.purchaseBoundCatalogDimensionAssignmentReady);
  document.documentElement.dataset.nativeHangerExactInstalledPlacementReady = String(nativeHangerSchedule.exactInstalledPlacementReady);
  ...
  document.documentElement.dataset.ab2RfaMetadataAuditReady = String(purchasedSupportComponents.ab2RfaMetadataAuditReady);
  document.documentElement.dataset.ab2QuoteBoundProductMetadataReady = String(purchasedSupportComponents.ab2QuoteBoundProductMetadataReady);
  document.documentElement.dataset.fig69RfaDownloadRejected = String(purchasedSupportComponents.fig69RfaDownloadRejected);
  ...
  document.documentElement.dataset.projectPipeSizeAssignmentForFig69Ready = String(nativeHangerSchedule.perLineNominalPipeSizeAssignmentReady);
  ...
  document.documentElement.dataset.sammyIgesTopologyAuditReady = String(purchasedSupportComponents.sammyIgesTopologyAuditReady);
  ...
  document.documentElement.dataset.exactAssemblyPartDefinitionsReady = String(purchasedSupportComponents.exactAssemblyPartDefinitionsReady);
  document.documentElement.dataset.exactAssemblySourceTrustReady = String(purchasedSupportComponents.exactAssemblySourceTrustReady);
  document.documentElement.dataset.exactAssemblyInstalledInstanceCoverageReady = String(purchasedSupportComponents.exactAssemblyInstalledInstanceCoverageReady);
  document.documentElement.dataset.exactAssemblyConnectionFitReady = String(purchasedSupportComponents.exactAssemblyConnectionFitReady);
  document.documentElement.dataset.exactAssemblyStructureAttachmentReady = String(purchasedSupportComponents.exactAssemblyStructureAttachmentReady);
  document.documentElement.dataset.exactAssemblySolidKernelReceiptReady = String(purchasedSupportComponents.exactAssemblySolidKernelReceiptReady);
  document.documentElement.dataset.exactAssemblySceneCollisionReceiptReady = String(purchasedSupportComponents.exactAssemblySceneCollisionReceiptReady);
  document.documentElement.dataset.exactAssemblyReleaseReady = String(purchasedSupportComponents.exactAssemblyReleaseReady);
  document.documentElement.dataset.indexedCadCorpusAuditReady = String(purchasedSupportComponents.indexedCadCorpusAuditReady);
  document.documentElement.dataset.manufacturerCadExternalRequestReady = String(purchasedSupportComponents.manufacturerCadExternalRequestReady);
