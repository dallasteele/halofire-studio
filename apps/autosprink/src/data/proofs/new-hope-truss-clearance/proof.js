...
try {
  ...
  document.querySelector('#purchased-support-component-status').textContent = `REJECTED FROM INSTALLED MODEL: quote 0133821 binds ${purchasedSupportComponents.metrics.purchasedSupportLineCount} support product lines / ${purchasedSupportComponents.metrics.purchasedSupportUnitCount} units. Official Victaulic AB2 and ASC/Anvil Fig. 69 Revit sources are hash-bound, with ${purchasedSupportComponents.metrics.fig69PublishedVariantCount} published Fig. 69 size variants. The Sammy quote line remains unresolved between different SWDR 516 and SWDR 1-1/2 threads and lengths. Project pipe-size assignment, RFA dimension audit, threads, mating, placement, and exact CAD for every remaining support are still required`;
