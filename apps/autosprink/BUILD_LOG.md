...
- Bound CMI.06 to pipe-067/source-edges 139-143 from canonical node 137 to 138. The four threaded outlet stations at 15, 87, 159, and 231 inches reconcile to plan heads 064, 063, 058, and 057 with maximum station residual 0.985164 inch. The grooved branch outlet at 246 inches binds the 2-1/2-inch pipe-065 branch and the 3-inch pipe-063 continuation at node 138; the canonical-junction station residual is 4.001712 inches and remains explicitly bounded below 5 inches.
- Bound approved RA2-2 calculation node 50 at local carrier elevation 20.5 feet and node 718 at head-057 elevation 21.5 feet to the same canonical plan node. Their 1-inch pipe leg is exactly one foot vertical with 3.583333 feet of reported fitting-equivalent length. No other CMI.06 outlet drop receives fabricated Z.
- Verified 132 New Hope/FP2.0 tests across 22 files, including 13 adversarial CMI.06 source/piece/topology/station/fitting/orientation/calculation/Z/sprinkler mutations. JavaScript syntax, JSON parsing, production Vite build, agent-loop guard, agentic-rules verifier, focused-marker scan, and `git diff --check` passed. GX10 Lean 4.13.0 verified the five-outlet station closure, exact 12-inch vertical delta, and fail-closed whole-layout theorem in 83 ms with return code 0.
- In-app browser verification decoded all 12 actual PDF images, showed the field-plan and listing crops at readable scale, reported every bounded CMI.06 fitting/vertical/Z gate true, retained all four whole-layout blockers, and produced zero console warnings/errors. Complete fitting topology, whole-system centerline Z, field-routed drum drips, the concealed source-feed path, proper layout, fabrication, and field release remain false.
- Next: continue line-by-line fitting/vertical-offset registration from the New Hope field/listing/FAB/calculation corpus, then grow only calculation-and-offset-closed centerline-Z paths. Do not infer either field-routed drum-drip path or the concealed riser-room continuation.
...
## 2026-07-14 - Building J source-underlay visual proof and ceiling installation envelope

- Retired the blank-background Building J roof/elevation/3D schematics as visual proof. The replacement proof renders all 68 registered heads and four exact drain wedges on protected roof-plan page 106, replays four exact roof vectors on protected section page 110, renders 20 exact ceiling-material polygons on protected RCP page 105, and projects the same protected roof-plan raster into the registered 3D coordinate frame with four shared anchors.
- Extracted and hash-bound the protected RCP DWG/PDF, approved FP-2 sprinkler schedule, and submitted TFP171 cut sheet. The source packet binds 13 pendents to 9-foot ceilings, two pendents to 10-foot ceilings, approved TY3231 pendent and TY3131 upright products, and seven upright heads to RCP-proved above-finished-ceiling domains.
- Preserved the critical claim boundary: all 68 source protection targets now close, and 15 TY3231 standard-pendent deflector values are available only as conditional manufacturer geometry. Exact installed head Z, source-generated placement, compliance, fabrication, and field release remain false.
- Verified 45/45 focused Building J tests across ten files, 20/20 ceiling-envelope adversarial mutations, 30/30 source-underlay adversarial mutations, Vite production build, Python compile, agent-loop guard, agentic rules, and real in-app-browser rendering with four decoded images and zero console warnings/errors. The GX10 host answered ping but TCP 8810 was unavailable and no local Lean executable existed, so the exact theorem and deterministic fallback evidence are recorded in the loop receipt without inventing a prover pass.

## 2026-07-17 - Cooperative 1881 issued A-121 roof-block boundary receipt

- Added a sealed-native-DWG extractor for all thirty `Roof_*_2` blocks in the issued A-121 roof plan. It records 31 closed, non-degenerate source boundaries (23 native HATCH paths and 8 closed roof-edge LWPOLYLINE loops) with their exact block/insert/entity handles, while rejecting source-hash drift, parser unknown entities, count drift, missing inserts, unsupported transforms, and unresolved/open/degenerate boundaries.
- Added focused adversarial coverage for closed source loops, dangling edge rejection, zero-area rejection, and the receipt's false-promotion boundary. The receipt explicitly keeps roof planes, slope directions, elevations, member association, head placement, pipe routing, clearance, fabrication, code, employee use, and VPS release closed.
- Visual proof: the initial tall-plan composition was rejected. The accepted replacement rotates only the proof viewport for legibility, leaves source coordinates unchanged, centers the plan geometry, and displays an evidence panel identifying exactly what is and is not proved.
- Verified with `npx vitest run` across the source-bound Cooperative 1881 suite (33 passed), two byte-identical extractor replays (`17D5B3D0188CAABBE51AEA25138AAC38B65388DEACCCE1A851764AC0575CB297`), `npm run build`, `C:/Python312/python.exe E:/ClaudeBot/scripts/verify_agentic_rules.py`, a Chrome screenshot inspection, and loop guard validation. `GX10_PROVER_UNAVAILABLE`: `http://127.0.0.1:8810/api/provers/health` could not be reached, so no formal proof is claimed.

## 2026-07-17 - Cooperative 1881 A-121 slope target region association

- Added an adversarial association receipt that canonicalizes only exact duplicate A-121 roof-block polygons. It retains all source aliases/handles but refuses to turn non-identical overlapping plan regions into one assumed physical face.
...
## 2026-07-17 - Cooperative 1881 A-307 C1 native source-view binding

- Added a sealed native A-307 view receipt that starts with the explicit A-121 `C1/A-307` cutting-plane reference, verifies the matching native NCS title attributes (`C1`, `A-307`, `LONGITUDINAL SECTION B`), and binds the title to exactly one model viewport using its documented paper-layout relationship. It never selects a source view from a matching sheet name or nearest geometry.
