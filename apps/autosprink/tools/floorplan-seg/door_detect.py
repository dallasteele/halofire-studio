...
def _save_outputs(
    ...
    *,
    ...
    json_path.write_text(json.dumps([_serialize_door(candidate) for candidate in deduped], indent=2))
...
def _run_detection(
    ...
    *,
    ...
    overlay_path, json_path = _save_outputs(
        raster_bgr,
        bbox,
        openings,
        deduped,
        out_dir,
        focus_bbox_px=focus_bbox_px,
    )
    doors = json.loads(json_path.read_text())
...
def run_door_detection(
    ...
    return _run_detection(Path(raster_path), Path(out_dir), focus_bbox_px=focus_bbox_px)


def run_door_detection_per_tower(
    raster_path: Path | str = DEFAULT_RASTER,
    out_dir: Path | str = DEFAULT_OUT_DIR,
) -> dict[str, Any]:
    raster_path = Path(raster_path)
    ...
    plan_bbox = _load_plan()["footprintBboxFt"]
    split = split_towers(raster_path, out_dir / "tower_split")
    tower_summaries: list[dict[str, Any]] = []
    aggregate_doors: list[dict[str, Any]] = []
    overlay = Image.open(raster_path).convert("RGB")
    draw = ImageDraw.Draw(overlay)

    for tower in split["towers"]:
        tower_bbox = tower["bbox"]
        focus_bbox_px = (
            int(tower_bbox["x"]),
            int(tower_bbox["y"]),
            int(tower_bbox["x"]) + int(tower_bbox["w"]),
            int(tower_bbox["y"]) + int(tower_bbox["h"]),
        )
        summary = _run_detection(
            raster_path,
            out_dir / f"tower_{tower['index']}",
            focus_bbox_px=focus_bbox_px,
        )
        tower_doors = json.loads(Path(summary["doorsPath"]).read_text())
        for door in tower_doors:
            door["towerIndex"] = int(tower["index"])
            aggregate_doors.append(door)
        tower_summaries.append(
            {
                "index": int(tower["index"]),
                "bbox": tower_bbox,
                "doorCount": len(tower_doors),
                "overlayPath": summary["overlayPath"],
                "doorsPath": summary["doorsPath"],
                "metrics": summary["metrics"],
            }
        )
        draw.rectangle(focus_bbox_px, outline=(144, 121, 255), width=4)

    aggregate_json_path = out_dir / "doors.json"
    aggregate_overlay_path = out_dir / "doors_overlay.png"
    aggregate_json_path.write_text(json.dumps(aggregate_doors, indent=2))

    for door in aggregate_doors:
        px = round((door["position"][0] - plan_bbox["minX"]) / (plan_bbox["maxX"] - plan_bbox["minX"]) * (overlay.width - 1))
        py = round((door["position"][1] - plan_bbox["minY"]) / (plan_bbox["maxY"] - plan_bbox["minY"]) * (overlay.height - 1))
        radius_px = max(
            8,
            round(
                door["width"] / ((plan_bbox["maxX"] - plan_bbox["minX"]) / (overlay.width - 1))
            ),
        )
        box = (px - radius_px, py - radius_px, px + radius_px, py + radius_px)
        color = (74, 190, 86) if door["confidence"] == "medium" else (245, 184, 65)
        draw.arc(
            box,
            start=door["swing"]["arcStartDeg"],
            end=door["swing"]["arcEndDeg"],
            fill=color,
            width=4,
        )
        leaf_theta = math.radians(door["swing"]["leafAngleDeg"])
        leaf_x = px + round(radius_px * math.cos(leaf_theta))
        leaf_y = py + round(radius_px * math.sin(leaf_theta))
        draw.line((px, py, leaf_x, leaf_y), fill=(58, 139, 255), width=3)
        draw.ellipse((px - 4, py - 4, px + 4, py + 4), fill=(255, 0, 0))

    overlay.save(aggregate_overlay_path)
    summary = {
        "rasterPath": str(raster_path),
        "overlayPath": str(aggregate_overlay_path),
        "doorsPath": str(aggregate_json_path),
        "doors": aggregate_doors,
        "towers": tower_summaries,
        "metrics": {
            "towerCount": len(tower_summaries),
            "doorCount": len(aggregate_doors),
            "doorCountsByTower": {str(tower["index"]): tower["doorCount"] for tower in tower_summaries},
            "rawCircles": sum(int(tower["metrics"]["rawCircles"]) for tower in tower_summaries),
            "candidateDoors": sum(int(tower["metrics"]["candidateDoors"]) for tower in tower_summaries),
        },
        "note": (
            "Best-effort raster door detector for the real 1881 page, run per tower crop. "
            "Candidates must still satisfy quarter-arc edge coverage, a leaf-like radial line, "
            "and adjacency to the committed wall runs."
        ),
    }
    (out_dir / "doors-summary.json").write_text(json.dumps(summary, indent=2))
    return summary
