...
def _apply_memory_priors(
    priors: dict[str, Any],
    *,
    current_walls: list[dict[str, Any]],
    current_doors: list[dict[str, Any]],
    vector_pool: dict[tuple[float, float, float, float], dict[str, Any]],
    wall_keys: set[tuple[float, float, float, float]],
    door_keys: set[tuple[int, int, int]],
    ...
    for entry in priors.get("local_plan_memory", []):
        if isinstance(entry, dict):
            structured_entries.append(entry)
    for entry in priors.get("brain_plan_memory", []):
        ...
        if isinstance(context, dict):
            structured_entries.append(
                {
                    "issueType": context.get("issue_type"),
                    "bbox_px": context.get("bbox_px"),
                }
            )
    ...
    for entry in structured_entries:
        ...
        if not isinstance(bbox_px, list) or len(bbox_px) != 4:
            continue
        issue_type = str(entry.get("issueType") or "")

        if issue_type == "parking-as-wall":
            kept: list[dict[str, Any]] = []
            for line in current_walls:
                if _line_intersects_px_region(line, bbox_px, plan_bbox, width, height):
                    removed_walls += 1
        ...
        if issue_type == "missed-wall":
            for key, candidate in vector_pool.items():
                if key in wall_keys:
                    continue
                if not _line_intersects_px_region(candidate, bbox_px, plan_bbox, width, height):
                    continue
        ...
        if issue_type == "missed-door":
            focus_summary = _load_module(HERE / "door_detect.py", "floorplan_seg_door_detect_prior").run_door_detection(
                raster_path,
                out_dir / "door_memory_focus",
                tuple(bbox_px),
            )
            for door in focus_summary["doors"]:
                ...
                if key in door_keys:
                    continue
                if not _door_in_px_region(door, bbox_px, plan_bbox, width, height):
                    continue
                current_doors.append(door)
                door_keys.add(key)
                added_doors += 1

    for entry in priors.get("global_priors", []):
        if not isinstance(entry, dict):
            continue
        context = entry.get("context")
        if not isinstance(context, dict):
            continue
        if context.get("issue_type") != "parking-as-wall":
            continue
        kept = []
        for line in current_walls:
            if line.get("parkingRegionHit"):
                removed_walls += 1
                wall_keys.discard(_line_key(line))
                continue
            kept.append(line)
...
def run_judge_loop(
    ...
    memory = _load_module(HERE / "plan_memory.py", "floorplan_seg_plan_memory")
    ...
    raster_size = Image.open(raster_path).size
    priors = memory.recall_plan_memory(plan_id="1881-p8")
    prior_application = _apply_memory_priors(
        priors,
    ...
    log: dict[str, Any] = {
        ...
        "memoryRecall": {
            "localPlanMemoryCount": len(priors.get("local_plan_memory", [])),
            "brainPlanMemoryCount": len(priors.get("brain_plan_memory", [])),
            "globalPriorsCount": len(priors.get("global_priors", [])),
            "localPath": priors.get("localPath"),
        },
        "memoryAppliedBeforeJudge": prior_application,
        "iterations": [],
    }
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        memory_results = memory.remember_iteration(
            plan_id="1881-p8",
            critique=iteration_log["critique"],
            changes_applied=iteration_log["changesApplied"],
            iteration=iteration,
        )
        iteration_log["memoryWrites"] = {
            "count": len(memory_results),
            "entries": [item["entry"] for item in memory_results],
        }
