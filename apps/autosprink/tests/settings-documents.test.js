...
describe('HaloFire settings + documentation upload/link API', () => {
  ...
  it('posts replacement-derived SAM31 section artifacts into LandScout and NameForge consumer intake smokes', async () => {
    ...
    async function postReplacement(consumer, reviewEvidenceId) {
      const res = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/actual-value-replacements`, {
        ...
        body: JSON.stringify({
          ...
          consumer,
          selected_sheet_ref: '1881://proposal-cooperative/sheet-7',
          selected_scale_ref: 'scale:1881-sheet-7:1-8in-1ft',
          selected_boundary_candidate_ref: 'candidate:1881-sheet-7-outline',
          source_file: 'Proposal-Cooperative 1881-Salt Lake City UT-9-18-25.xlsx',
    ...
    const smokeSprinklerDecisionRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet/decision`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        issue_type: 'sam31_consumer_intake_obstruction_clash_review',
        ...
        review_ref: 'employee-review://nameforge/smoke-sprinkler/obstruction',
        screenshot_ref: 'screenshot://nameforge/smoke-sprinkler/obstruction.png',
        reviewed_values: {
          obstruction_candidates: [{ id: 'obs-1', action: 'keep_clearance' }],
          sleeve_or_firestop_candidates: [{ id: 'sleeve-1', action: 'employee_review' }],
          confidence: 0.66,
        },
        notes: 'Smoke-derived sprinkler review decision for obstruction lane',
      }),
    });
    expect(smokeSprinklerDecisionRes.status).toBe(201);
    const smokeSprinklerDecision = await smokeSprinklerDecisionRes.json();
    expect(smokeSprinklerDecision).toEqual(expect.objectContaining({
      ...
      source_queue_item_id: expect.stringContaining(':sprinkler:'),
      source_section_to_artifacts_consumer_intake_smoke_evidence_id: consumerIntakeSmokes.nameforge.evidence_id,
      source_halofire_sam31_consumer_intake_smoke_followup_review_evidence_id: followupReview.id,
      source_sam31_actual_value_replacement_evidence_id: consumerIntakeSmokes.nameforge.source_sam31_actual_value_replacement_evidence_id,
      source_openclaw_sam31_consumer_review_evidence_id: consumerIntakeSmokes.nameforge.source_openclaw_sam31_consumer_review_evidence_id,
    ...
    expect(smokeSprinklerDecision.reviewed_values).toEqual(expect.objectContaining({
      confidence: 0.66,
      obstruction_candidates: expect.arrayContaining([expect.objectContaining({ id: 'obs-1' })]),
    }));
    expect(smokeSprinklerDecision.source_refs).toEqual(expect.arrayContaining([
      expect.objectContaining({
        evidence_id: consumerIntakeSmokes.nameforge.evidence_id,
        evidence_type: 'openclaw_sam31_section_to_artifacts_consumer_intake_smoke',
      }),
      ...
      expect.objectContaining({
        ...
        source_ref: 'employee-review://nameforge/smoke-sprinkler/obstruction',
      }),
    ]));
    expect(smokeSprinklerDecision.blocked_claims).toEqual(expect.arrayContaining([
      'permit_ready',
      'fabrication_ready',
      'AHJ_approval',
      'professional_approval',
    ...
    const reviewedSprinklerPacketRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(reviewedSprinklerPacketRes.status).toBe(200);
    const reviewedSprinklerPacket = await reviewedSprinklerPacketRes.json();
    expect(reviewedSprinklerPacket.issue_seeds).toEqual(expect.arrayContaining([
      expect.objectContaining({
        issue_type: 'sam31_consumer_intake_obstruction_clash_review',
        supported_sprinkler_review_lane: 'obstruction_or_clash_review',
    ...
    const smokeReplayArtifactRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet/decision/${smokeSprinklerDecision.id}/preliminary-replay`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(smokeReplayArtifactRes.status).toBe(200);
    const smokeReplayArtifact = await smokeReplayArtifactRes.json();
    expect(smokeReplayArtifact).toEqual(expect.objectContaining({
    ...
    expect(smokeReplayArtifact.replay_inputs).toEqual(expect.objectContaining({
      artifact_type: 'halofire.sam31_sprinkler_review_preliminary_replay_inputs.v1',
      source_halofire_sam31_sprinkler_review_decision_evidence_id: smokeSprinklerDecision.id,
    }));
    expect(smokeReplayArtifact.replay_output).toEqual(expect.objectContaining({
    ...
    expect(smokeReplayArtifact.replay_output.issue_candidates).toEqual(expect.arrayContaining([
      expect.objectContaining({
        source_field: 'obstruction_candidates',
        value: expect.objectContaining({ id: 'obs-1' }),
      }),
    ...
    const smokeReplayFollowupRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet/decision/${smokeSprinklerDecision.id}/preliminary-replay/followup`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        followup_decision: 'confirmed_internal_obstruction_clash_packet',
        reviewer_name: 'Smoke Replay Reviewer',
        review_ref: 'employee-review://nameforge/smoke-preliminary-replay/followup',
        screenshot_ref: 'screenshot://nameforge/smoke-preliminary-replay/followup.png',
        console_log_ref: 'console://nameforge/smoke-preliminary-replay/followup.log',
        issue_decisions: [
          {
            source_field: 'obstruction_candidates',
            source_index: 0,
            decision: 'confirmed_internal_obstruction_clash_packet',
            target_packet_lane: 'obstruction_or_clash_review',
            packet_ref: 'packet://nameforge/obstruction-clash/obs-1',
          },
          {
            source_field: 'sleeve_or_firestop_candidates',
            source_index: 1,
            decision: 'confirmed_internal_sleeve_firestop_packet',
            target_packet_lane: 'sleeve_or_firestop_candidate_review',
            packet_ref: 'packet://nameforge/sleeve-firestop/sleeve-1',
          },
        ],
      }),
    });
    expect(smokeReplayFollowupRes.status).toBe(201);
    const smokeReplayFollowup = await smokeReplayFollowupRes.json();
    expect(smokeReplayFollowup).toEqual(expect.objectContaining({
      artifact_type: 'halofire.sam31_sprinkler_preliminary_replay_followup_decision.v1',
    ...
    expect(smokeReplayFollowup.packet_queue_items).toEqual(expect.arrayContaining([
      expect.objectContaining({
      ...
      expect.objectContaining({
        artifact_type: 'halofire.sam31_sleeve_firestop_packet_queue_item.v1',
        source_section_to_artifacts_consumer_intake_smoke_evidence_id: consumerIntakeSmokes.nameforge.evidence_id,
        source_halofire_sam31_sprinkler_review_decision_evidence_id: smokeSprinklerDecision.id,
        source_field: 'sleeve_or_firestop_candidates',
        target_packet_lane: 'sleeve_or_firestop_candidate_review',
    ...
    expect(smokeReplayFollowup.blocked_claims).toEqual(expect.arrayContaining([
      'permit_ready',
      'fabrication_ready',
      'AHJ_approval',
      'professional_approval',
    ...
    const followedUpSprinklerPacketRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(followedUpSprinklerPacketRes.status).toBe(200);
    const followedUpSprinklerPacket = await followedUpSprinklerPacketRes.json();
    expect(followedUpSprinklerPacket.issue_seeds).toEqual(expect.arrayContaining([
      expect.objectContaining({
        source_section_to_artifacts_consumer_intake_smoke_evidence_id: consumerIntakeSmokes.nameforge.evidence_id,
    ...
    const smokeObstructionPacketRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet/decision/${smokeSprinklerDecision.id}/preliminary-replay/followup/${smokeReplayFollowup.id}/packet/0`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(smokeObstructionPacketRes.status).toBe(200);
    const smokeObstructionPacket = await smokeObstructionPacketRes.json();
    expect(smokeObstructionPacket).toEqual(expect.objectContaining({
    ...
    expect(smokeObstructionPacket.source_refs).toEqual(expect.arrayContaining([
      expect.objectContaining({
        evidence_id: consumerIntakeSmokes.nameforge.evidence_id,
        evidence_type: 'openclaw_sam31_section_to_artifacts_consumer_intake_smoke',
      }),
    ...
    const smokeSleevePacketRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet/decision/${smokeSprinklerDecision.id}/preliminary-replay/followup/${smokeReplayFollowup.id}/packet/1`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    expect(smokeSleevePacketRes.status).toBe(200);
    const smokeSleevePacket = await smokeSleevePacketRes.json();
    expect(smokeSleevePacket).toEqual(expect.objectContaining({
      artifact_type: 'halofire.sam31_sleeve_firestop_packet.v1',
      source_packet_queue_item_artifact_type: 'halofire.sam31_sleeve_firestop_packet_queue_item.v1',
      source_section_to_artifacts_consumer_intake_smoke_evidence_id: consumerIntakeSmokes.nameforge.evidence_id,
      source_halofire_sam31_consumer_intake_smoke_followup_review_evidence_id: followupReview.id,
      source_halofire_sam31_sprinkler_review_decision_evidence_id: smokeSprinklerDecision.id,
      source_followup_decision_evidence_id: smokeReplayFollowup.id,
    ...
    const smokePacketReviewRes = await request(`/api/projects/${encodeURIComponent(projectName)}/openclaw/sam31/section-to-artifacts-consumer-intake-smoke/${consumerIntakeSmokes.nameforge.evidence_id}/sprinkler-review-packet/decision/${smokeSprinklerDecision.id}/preliminary-replay/followup/${smokeReplayFollowup.id}/packet/0/review`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        review_decision: 'accepted_internal_alpha_packet',
        reviewer_name: 'Smoke Packet Reviewer',
        review_ref: 'employee-review://nameforge/smoke-packet/obstruction',
        signed_packet_ref: 'packet-review://nameforge/smoke-packet/obstruction/signed',
        marked_up_screenshot_ref: 'screenshot://nameforge/smoke-packet/obstruction.png',
        notes: 'Internal alpha smoke obstruction packet reviewed; regulated claims still blocked.',
      }),
    });
    expect(smokePacketReviewRes.status).toBe(201);
    const smokePacketReview = await smokePacketReviewRes.json();
    expect(smokePacketReview).toEqual(expect.objectContaining({
    ...
    expect(tool.halofire_api_actions.consumer_intake_smoke_preliminary_replay).toEqual(expect.objectContaining({
      method: 'GET',
      ...
      consumes: 'halofire.sam31_sprinkler_review_preliminary_replay_inputs.v1',
      produces: 'halofire.sam31_sprinkler_preliminary_replay_artifact.v1',
      target_application: 'halo_fire',
      use_for_claims: false,
      claim_gate_effect: 'no_claims_cleared',
    }));
    expect(tool.halofire_api_actions.consumer_intake_smoke_preliminary_replay_followup).toEqual(expect.objectContaining({
      method: 'POST',
      ...
      consumes: 'halofire.sam31_sprinkler_preliminary_replay_artifact.v1',
      produces: 'halofire.sam31_sprinkler_preliminary_replay_followup_decision.v1',
      downstream_resolver_artifact_types: expect.arrayContaining([
        'halofire.sam31_obstruction_clash_packet_queue_item.v1',
        'halofire.sam31_sleeve_firestop_packet_queue_item.v1',
      ]),
      target_application: 'halo_fire',
      use_for_claims: false,
      claim_gate_effect: 'no_claims_cleared',
    }));
    expect(tool.halofire_api_actions.consumer_intake_smoke_preliminary_replay_followup_packet).toEqual(expect.objectContaining({
      method: 'GET',
      ...
      consumes: 'halofire.sam31_sprinkler_preliminary_replay_followup_decision.v1',
      produces_any: expect.arrayContaining([
        'halofire.sam31_obstruction_clash_packet.v1',
        'halofire.sam31_sleeve_firestop_packet.v1',
      ]),
      target_application: 'halo_fire',
      use_for_claims: false,
      claim_gate_effect: 'no_claims_cleared',
    }));
    expect(tool.halofire_api_actions.consumer_intake_smoke_preliminary_replay_followup_packet_review).toEqual(expect.objectContaining({
      method: 'POST',
      ...
      consumes_any: expect.arrayContaining([
        'halofire.sam31_obstruction_clash_packet.v1',
        'halofire.sam31_sleeve_firestop_packet.v1',
      ]),
      produces: 'halofire.sam31_sprinkler_followup_packet_review_decision.v1',
      target_application: 'halo_fire',
      use_for_claims: false,
      claim_gate_effect: 'no_claims_cleared',
    }));
