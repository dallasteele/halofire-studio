...
describe('New Hope complete cross-main drainage direction', () => {
  it('orients the exact 31-node, 30-edge cross-main tree toward three source-bound outlets', () => {
    const result = evaluateNewHopeCrossMainDrainage(inputs);
    expect(result.status).toBe('passed');
    expect(result.metrics).toEqual({
      canonicalNodeCount: 31,
      canonicalEdgeCount: 30,
      directedEdgeCount: 30,
    ...
    expect(result.terminalNodeIds).toEqual(['canonical-node-002', 'canonical-node-054', 'canonical-node-056']);
