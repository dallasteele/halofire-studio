...
const packet = read('dillon-vertical-registration.json'); const sourceGeometry = read('dillon-dwg-source-geometry.json'); const bidGeometry = read('dillon-completed-bid-geometry.json'); const floorModel = read('dillon-floor-by-floor-model.json'); const slopedCalibration = read('submitted-sloped-ceiling-calibration.dillon.json'); const rcpFaceRegistry = read('dillon-rcp-vector-face-registry.json');
const dependencies = { sourceGeometry, bidGeometry, floorModel, slopedCalibration, rcpFaceRegistry };
...
describe('Dillon partial source-bound vertical registration', () => {
  it('assigns only source-supported Z and leaves the rest unresolved', () => {
    expect(result.status).toBe('passed'); expect(result.counts).toEqual({ totalHeads: 76, sourceAssignedHeads: 34, unresolvedHeads: 42, totalPipeSegments: 67, sourceAssignedPipeSegments: 11, unresolvedPipeSegments: 56 });
  ...
  it('builds 3D coordinates only for assigned elements', () => {
    const model = buildDillonVerticalModel(result); expect(model.status).toBe('passed'); expect(model.heads).toHaveLength(34); expect(model.pipes).toHaveLength(11); expect(model.heads.some((head) => head.surfaceKind === 'sloped-ceiling')).toBe(true); expect(model.heads.some((head) => head.sourceFaceId)).toBe(true); expect(model.complete).toBe(false);
    const view = renderDillonVerticalElevationView(model); expect(view.svg).toContain('34/76 heads + 11/67 pipes have source-bound Z'); expect(view.svg).toContain('unresolved elements are omitted');
  });
  it.each([
    ...
    ['RCP face reassignment', (value) => { const row = value.sheets.flatMap((sheet) => sheet.headAssignments).find((entry) => entry.sourceFaceId); row.sourceFaceId = 'fabricated-face'; }],
  ...
  it('blocks RCP vector-face registry substitution', async () => { const changed = structuredClone(rcpFaceRegistry); changed.receiptSha256 = '0'.repeat(64); const blocked = await validateDillonVerticalRegistration(packet, { ...dependencies, rcpFaceRegistry: changed }); expect(blocked.issues.map((entry) => entry.code)).toContain('DILLON_VERTICAL_RCP_FACE_SOURCE_MISMATCH'); });
});
