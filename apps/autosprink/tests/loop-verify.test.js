...
test('verifyPass can derive door locations from wall openings on existing wall solids', () => {
  const model = {
    footprint: { polygon: rect(0, 0, 20, 10), contourLengthFt: 60 },
    shell: { polygon: rect(0, 0, 20, 10) },
    planInkContourLengthFt: 60,
    grid: { cols: 1, rows: 1 },
    rooms: [{ name: 'Lobby', polygon: rect(0, 0, 20, 10) }],
    solids: [
      {
        kind: 'wall',
        a: [0, 0],
        b: [20, 0],
        openings: [{ offsetFt: 8, widthFt: 4, heightFt: 7, type: 'door' }],
      },
      { kind: 'wall', a: [20, 0], b: [20, 10] },
      { kind: 'wall', a: [20, 10], b: [0, 10] },
      { kind: 'wall', a: [0, 10], b: [0, 0] },
      { kind: 'column', x: 10, y: 5 },
    ],
  };
  const result = verifyPass(model, 'openings');
  assert.equal(result.ok, true);
  assert.deepEqual(result.violations, []);
});
