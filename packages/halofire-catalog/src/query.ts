...
import type { CatalogEntry, ComponentCategory } from './types.js'

/** Return all entries in a given category. */
export function findByCategory(category: ComponentCategory): readonly CatalogEntry[] {
  return CATALOG.filter((e) => e.category === category)
}

/** Case-insensitive name substring match. */
export function findByName(query: string): readonly CatalogEntry[] {
...
export function findBySku(sku: string): CatalogEntry {
...
export function findHeadsByKFactor(kFactor: number): readonly CatalogEntry[] {
  return CATALOG.filter(
...
export function findPipesBySize(pipeSizeIn: number): readonly CatalogEntry[] {
  return CATALOG.filter(
