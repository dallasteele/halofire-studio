...
describe('provenance artifacts', () => {
  ...
  test('Tyco TY3251 keeps the manufacturer-backed family separate from the Ferguson salvage proxy', () => {
    ...
    const sidewallHorizontal = byKey.get('sidewall_horizontal')
    ...
    expect(sidewallHorizontal?.source_license.source_file_ref).toContain('tyco_lfii_hsw_tfp417.pdf')
    expect(sidewallHorizontal?.family_contract.model_status).toBe('proxy')
    expect(sidewallHorizontal?.family_contract.ifc_path).toBeNull()
    expect(sidewallHorizontal?.family_contract.dxf_path).toBeNull()

    const sidewallDry = byKey.get('sidewall_dry')
    expect(sidewallDry).toBeDefined()
    expect(sidewallDry?.source_kind).toBe('manufacturer')
    expect(sidewallDry?.model_status).toBe('proxy')
    expect(sidewallDry?.source_license.source_file_ref).toContain('reliable_dh56_bulletin016.pdf')
    expect(sidewallDry?.family_contract.model_status).toBe('proxy')
    expect(sidewallDry?.family_contract.ifc_path).toBeNull()
    expect(sidewallDry?.family_contract.dxf_path).toBeNull()
